...
export function UserPicker({ initial = "" }: { initial?: string }) {
  ...
  const [results, setResults] = useState<
    { id: number; label: string; discord_id: string | null }[] | null
  >(null);
  ...
  const open = (userId: number) => router.push(`/admin/users?userId=${userId}` as Route);

  const onSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const trimmed = q.trim();
    if (!trimmed) return;
    // Numeric input → treat as a user id directly.
    if (/^\d+$/.test(trimmed)) {
      open(Number(trimmed));
      return;
    }
    ...
    <div className="space-y-3">
      <form onSubmit={onSubmit} className="flex gap-2">
        <input
          ...
          placeholder="Username, Discord ID, or numeric user ID…"
      ...
      {pending ? (
        ...
        results && (
          <ul className="divide-osrs-bronze/20 border-osrs-bronze/20 divide-y rounded border">
            {results.length === 0 ? (
              <li className="text-osrs-parchment-dark/60 px-3 py-2.5 text-sm">No users found.</li>
            ) : (
              results.map((u) => (
                <li key={u.id}>
                  <button
                  ...
                  >
                    <span>
                      {u.label}
                      <span className="text-osrs-parchment-dark/50 ml-2 text-xs">#{u.id}</span>
                    </span>
                    {u.discord_id && (
                      <span className="text-osrs-parchment-dark/60 text-xs">{u.discord_id}</span>
                    )}
                  </button>
                </li>
              ))
